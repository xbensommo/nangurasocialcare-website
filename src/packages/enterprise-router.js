...
// Default Error Routes Generator
  errorRoutes: (loader) => [
    { path: '/400', name: '400', component: loader('400'), props: route => ({ code: '400', message: route.query.message, description: route.query.reason }) },
...
/** * BUG FIX: Instead of 'return next()', we only 'next()' if the route 
* is NOT protected. If it IS protected and we aren't initialized, 
* we must wait or redirect, not bypass.
*/
       /* if (!authInitialized && isProtected) {
              // If not initialized, we cannot verify roles/auth. 
              // Redirecting to 401/Login or a generic 'loading' state is safer than next()
            return next({ name: '401', query: { redirect: to.fullPath, reason: 'Initializing...' }})
          }*/
...
// Role Check
        ...
        if (routeRoles) {
// If roles are required but no user exists (even if auth is initialized)
          if (!currentUser) {
