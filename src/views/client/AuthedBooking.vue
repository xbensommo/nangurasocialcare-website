...
<!-- Authentication Prompt - Only show for non-organization bookings AND when user is NOT logged in -->
<div v-if="!store.currentUser && currentStep > 1 && !isOrganizationBooking && currentStep !== 5 && !isUserLoggedIn" class="mb-6 p-4 bg-amber-50 border border-amber-200 rounded-2xl flex items-center justify-between">
...
<!-- Logged-in User Indicator - Show when user IS logged in and on step 2+ -->
<div v-if="store.currentUser && currentStep > 1 && !isOrganizationBooking && currentStep !== 5" class="mb-6 p-4 bg-green-50 border border-green-200 rounded-2xl flex items-center justify-between">
  <div class="flex items-center gap-3">
    <i class="fas fa-check-circle text-green-600"></i>
    <p class="text-sm text-green-800">
      Signed in as <span class="font-bold">{{ store.currentUser.displayName || store.currentUser.email }}</span>
    </p>
  </div>
  <button @click="handleSignOut" class="px-6 py-2 bg-white text-green-600 rounded-full text-xs font-bold uppercase tracking-wider hover:bg-green-50 transition-colors border border-green-200">
    Not you? Sign Out
  </button>
</div>
...
<div v-else-if="currentStep === 2" class="p-8 md:p-12">
  ...
  <!-- Logged-in user info for non-organization bookings -->
  <div v-if="store.currentUser && !isOrganizationBooking" class="mb-6 p-4 bg-green-50 border border-green-200 rounded-2xl">
    <div class="flex items-center gap-3">
      <i class="fas fa-user-check text-green-600"></i>
      <p class="text-sm text-green-800">
        Booking as <span class="font-bold">{{ store.currentUser.displayName || store.currentUser.email }}</span>
      </p>
    </div>
  </div>
...
<div v-else-if="currentStep === 3" class="p-8 md:p-12">
  ...
  <div v-if="store.currentUser && !isOrganizationBooking" class="mb-6 p-4 bg-green-50 border border-green-200 rounded-2xl">
    <div class="flex items-center gap-3">
      ...
      <div>
        <p class="text-sm font-bold text-green-800">Welcome back, {{ store.currentUser.displayName || 'Valued Client' }}!</p>
        <p class="text-xs text-green-600">Your information has been pre-filled. Please verify and update if needed.</p>
      </div>
    </div>
  </div>
  ...
  <div v-else class="grid md:grid-cols-2 gap-6">
    <div class="space-y-2">
      ...
      <input :readonly="!!store.currentUser" v-model="form.fullName" type="text" class="w-full px-6 py-4 rounded-2xl bg-[#FAFAF9] border-transparent focus:bg-white focus:border-[var(--color-secondary)] transition-all outline-none text-sm disabled:opacity-70" :placeholder="store.currentUser ? 'Auto-filled from account' : 'Your full name'">
    </div>
    ...
    <div class="md:col-span-2 space-y-2">
      <label class="text-[10px] uppercase font-bold tracking-widest text-gray-400 ml-2">Email Address</label>
      <input :readonly="!!store.currentUser" v-model="form.email" type="email" class="w-full px-6 py-4 rounded-2xl bg-[#FAFAF9] border-transparent focus:bg-white focus:border-[var(--color-secondary)] transition-all outline-none text-sm" :placeholder="store.currentUser ? 'Auto-filled from account' : 'your@email.com'">
    </div>
...
<div v-else-if="currentStep === 4" class="p-8 md:p-12">
  ...
  <div class="bg-[#FAFAF9] rounded-3xl p-8 mb-8 space-y-6">
    ...
    <div v-if="store.currentUser && !isOrganizationBooking" class="flex items-start gap-3 bg-green-50/50 p-4 rounded-2xl">
      <i class="fas fa-shield-alt text-green-600 mt-1"></i>
      <p class="text-[11px] text-gray-600 leading-relaxed">
        <span class="font-bold text-green-700">Verified Account:</span> This booking will be linked to your account ({{ store.currentUser.email }}). You can manage it from your dashboard.
      </p>
    </div>
...
// --- State Management ---
  ...
  const hasGoogleSignIn = ref(true);
...
// Computed property to check if user is logged in
  const isUserLoggedIn = computed(() => !!store.currentUser);
  ...
  const confirmBooking = async () => {
    try {
      ...
      const payload = {
        ...
        userId: store.currentUser?.uid || null // Link to user if logged in
      };
...
onMounted(async () => {
  ...
  if (store.currentUser) {
    form.fullName = store.currentUser.fullName || '';
    form.email = store.currentUser.email || '';
