    ...
    <div class="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-8">
      ...
      <aside class="lg:col-span-3 space-y-6 hidden lg:block">
        <div class="bg-white rounded-[32px] border border-gray-100 p-8 shadow-sm">
          <div class="flex flex-col items-center text-center mb-8">
            ...
            <h3 class="font-serif text-xl text-[var(--color-primary)]">{{ store.currentUser?.fullName}}</h3>
            <p class="text-[10px] text-[var(--color-secondary)] font-bold uppercase tracking-[0.2em] mt-1">{{ (store.currentUser?.isDeleted) ? 'Account Close' : 'Active Client' }}</p>
          </div>
          ...
          <div class="space-y-4 pt-6 border-t border-gray-50">
            <div class="flex justify-between items-center text-[10px] font-bold uppercase tracking-widest">
              ...
              <span class="text-[var(--color-primary)]">
                    {{
                      store.sessions.aggregatedCount
                    }}
              </span>
            </div>
            <div class="flex justify-between items-center text-[10px] font-bold uppercase tracking-widest">
      ...
      <aside class="lg:col-span-3 space-y-8">
        <div class="bg-[var(--color-primary)] rounded-[32px] p-8 text-white relative overflow-hidden">
          <h3 class="text-[9px] uppercase tracking-[0.4em] text-[var(--color-secondary)] font-bold mb-8 relative z-10">Recent Sessions</h3>
          
          <div class="space-y-8 relative z-10">
            <div v-for="res in pastSessions" class="relative pl-6 border-l border-white/10">
              <div class="absolute -left-[4.5px] top-0 w-2 h-2 rounded-full bg-[var(--color-secondary)]"></div>
              <p class="text-[8px] uppercase tracking-widest text-white/40 font-bold mb-1">{{ res.startDate }} • COMPLETED</p>
...
/**
 * query availableServices from firestore, as store.servicesActions.fetchByFilters({
 *  status: 'available'
 * })
 * 
 * */
...
// Make this showable only if its loaded with data else do not show it
let pastSessions = [];

//const pastSessions =  computed( () => store.sessions.items || [])

onMounted( async() =>{
 pastSessions = await store.sessionsActions.fetchByFilters({
    status: 'completed', clientId: store.currentUser.uid
  });

 await store.sessionsActions.getAggregatedCount({
  status: 'confirmed', clientId: store.currentUser.uid
 })
 /**
  * Here i want to store the client sessions history in clients collections, 
  * we just query the clients collection and we are done, we get the total sessions, when is the next sessions, recent sessions,
  * */
})
</script>
